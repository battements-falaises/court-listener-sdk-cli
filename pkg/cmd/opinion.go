...
func handleOpinionsRetrieve(ctx context.Context, cmd *cli.Command) error {
	...
	return ShowJSON(os.Stdout, os.Stderr, "opinions retrieve", obj, format, explicitFormat, transform)
}
...
func handleOpinionsList(ctx context.Context, cmd *cli.Command) error {
	...
	if format == "raw" {
		...
		obj := gjson.ParseBytes(res)
		return ShowJSON(os.Stdout, os.Stderr, "opinions list", obj, format, explicitFormat, transform)
	} else {
		...
		return ShowJSONIterator(os.Stdout, os.Stderr, "opinions list", iter, format, explicitFormat, transform, maxItems)
	}
}
